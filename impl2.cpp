...
struct json_as_xml : nlohmann::json_sax<json> {
    ...
    // - instance of fifo<any_t>
    // - thread instance to hold formatting thread
    // - flag to signal formatting thread that it's done.

    json_as_xml() {
        // TODO start formatting thread here that consumes fifo in a while loop until parsing is 'done'.
    }

    ~json_as_xml() {
        post(done_t{}); // signal that parsing is done;

        // TODO wait until formatting thread is finished.
    }
    ...
    void dump(done_t&&) {
        // When you get here you know that parsing is done because done_t is only posted from the destructor
        // TODO This is good place to set the flag to break out of the main loop of the formatting thread.
    }
    ...
    bool post(any_t&& value) {
        // This function is called upon every SAX parser call.
        // TODO instead of dump the passed value immediately, push the value to the fifo.
        // TODO the dump()-call should be moved to the main loop of formatting thread.
        dump(std::move(value));
        return true;
    }
